...
*/
    ...
    final File currentFile = File(path.join(home.path, 'counter_app1.rfwtxt'));
    final HttpClientResponse client = await (await HttpClient().getUrl(Uri.parse('$urlPrefix/counter_app1.rfwtxt'))).close();
    await currentFile.writeAsBytes(await client.expand((List<int> chunk) => chunk).toList());

    final File mainFile = File(path.join(home.path, 'rfwDemo.rfw'));
    final String rfwDemo = currentFile.readAsStringSync();
    mainFile.writeAsBytesSync(encodeLibraryBlob(parseLibraryFile(rfwDemo)));

    if(await mainFile.exists()){
      print('mainFile hai');
    }
    try{
      _runtime.update(const LibraryName(<String>['main']), decodeLibraryBlob(await mainFile.readAsBytes()));
      setState(() {
        _ready = true;
      });
    }catch(e){
